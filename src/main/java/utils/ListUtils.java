...
import com.csii.bank.core.base.support.aggregate.InvalidOperationException;
...
public class ListUtils {
		...
		Collections.sort(list, new Comparator<T>() {
			...
			public int compare(T a, T b) {
				...
				try {
					...
					e.printStackTrace();
					throw new InvalidOperationException("ListUtils排序出错");
				}
				return ret;
			}
		});
	 ...
	 * @throws Exception
	 */
	private static <T> int compareObject(final String sortname, final boolean isAsc, T a, T b) throws Exception {
		int ret;
	...
	/**
	 * 获取指定对象的指定属性值(取出private,protected的限制)
	 * 
	 * @param obj
	 *            属性名称所在对象
	 * @param fieldName
...
//		Field field = obj.getClass().getDeclaredField(fieldName);
		Field[] fields = ReflectUtil.getAllFields(obj);
		Field field = null;
		for (Field f : fields) {
